...
// Spectral Kubelka-Munk pigment mixing for the watercolor wet interaction.
//
// Given a backdrop texture (the paint already on the canvas, after the wet
// dilute/push) and a wash texture (the new watercolor stroke), this mixes the
// two like real transparent pigments: blue under a yellow wash reads green,
// not the muddy grey that alpha/multiply blending gives. Each colour is lifted
// to a 38-band reflectance spectrum, mixed via Kubelka-Munk weighted by how
// much pigment is present, then projected back to sRGB.
//
// The 38-band spectral core (spectral_* below) is adapted from spectral.js by
// Ronald van Wijnen, MIT License, Copyright (c) 2025. Constants verbatim.

precision highp float;

uniform vec2 uSize;
uniform float uWashGain;     // how strongly the wash pigment carries in the mix
uniform float uDarkHold;     // how much near-neutral darks resist being tinted
uniform sampler2D uBackdrop; // existing paint (premultiplied)
uniform sampler2D uWash;     // new watercolor stroke (premultiplied)
...
float spectral_KM(float KS) {
  // Algebraically 1 + KS - sqrt(KS^2 + 2*KS), but written as its reciprocal
  // conjugate so it stays stable in 32-bit float: the direct form suffers
  // catastrophic cancellation for large KS (near-black pigment), producing NaN
  // speckle. This form -> 0 smoothly instead.
  return 1.0 / (1.0 + KS + sqrt(KS * KS + 2.0 * KS));
}

// Mix two sRGB pigments by amount f1/f2 (each roughly a concentration in
// [0,1]). lumFloor1 raises color1's effective concentration weight so dark
// pigments hold their ground. Returns the mixed sRGB colour.
vec3 pigmentMix(vec3 color1, float f1, float lumFloor1, vec3 color2, float f2) {
  ...
  float luminance1 = max(lumFloor1, spectral_reflectance_to_xyz(R1)[1]);
  float luminance2 = spectral_reflectance_to_xyz(R2)[1];

  float R[SPECTRAL_SIZE];

  float concentration1 = f1 * f1 * luminance1;
  float concentration2 = f2 * f2 * luminance2;
  float totalConcentration = concentration1 + concentration2;

  for (int i = 0; i < SPECTRAL_SIZE; i++) {
    float ksMix = spectral_KS(R1[i]) * concentration1 + spectral_KS(R2[i]) * concentration2;
...
void main() {
  ...
  vec4 b = texture(uBackdrop, uv); // premultiplied
  vec4 w = texture(uWash, uv);     // premultiplied

  float ab = b.a;
  float aw = w.a;

  // No wash here: leave the backdrop untouched.
  if (aw <= 0.0009) {
    fragColor = b;
    return;
  }

  vec3 cw = w.rgb / aw; // unpremultiplied pigment colour

  // Bare surface underneath: just deposit the translucent wash (nothing to mix
  // with). Keeps the transition to painted areas continuous.
  if (ab <= 0.0009) {
    fragColor = vec4(cw * aw, aw);
    return;
  }

  vec3 cb = b.rgb / ab; // unpremultiplied backdrop pigment

  // Near-neutral darks (low chroma) resist tinting; saturated colours (e.g. a
  // blue that should read green under yellow) are left untouched.
  float chroma = max(cb.r, max(cb.g, cb.b)) - min(cb.r, min(cb.g, cb.b));
  float lumFloor = uDarkHold * (1.0 - chroma);

  // Mix pigments weighted by how much of each is present. uWashGain lets the
  // wash pigment carry more weight (stronger, more saturated mixing).
  vec3 mixed = pigmentMix(cb, ab, lumFloor, cw, aw * uWashGain);

  // Coverage builds up like layered washes (standard over).
  float ao = ab + aw * (1.0 - ab);
  fragColor = vec4(mixed * ao, ao);
}
